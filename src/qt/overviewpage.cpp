...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::ICC)
    {
    }
...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeICCAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeICCAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeICCAmount = strAnonymizeICCAmount.remove(strAnonymizeICCAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeICCAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeICCAmount * COIN) nMaxToAnonymize = nAnonymizeICCAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeICCAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeICCAmount));
        strAnonymizeICCAmount = strAnonymizeICCAmount.remove(strAnonymizeICCAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeICCAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
