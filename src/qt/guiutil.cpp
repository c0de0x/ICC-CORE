...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a ICC address (e.g. %1)").arg("GVXmy14r8hmuzt1d5ygnqzZfbuomg1XLmF"));
#endif
...
{
    // return if URI is not valid or is no ICC: URI
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert icc:// to icc:
    //
    //    Cannot handle this later, because icc:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open icc.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "ICC.lnk";
}
...
{
    return GetAutostartDir() / "icc.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a icc.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the icc app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add icc app to startup item list
